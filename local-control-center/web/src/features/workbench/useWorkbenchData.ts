...
const deliveryStageBlueprints = [
	...
	{
		id: 'architecture',
		labelKey: 'app.workbench.delivery.architecture',
		label: 'Architecture and environment',
		owner: 'Architecture + DevOps',
	},
	{
		id: 'implementation',
		labelKey: 'app.workbench.delivery.implementation',
		label: 'Implementation',
		owner: 'Engineering',
	},
	{
		id: 'validation',
		labelKey: 'app.workbench.delivery.validation',
		label: 'QA and security',
		owner: 'QA + Security',
	},
	{
		id: 'delivery',
		labelKey: 'app.workbench.delivery.delivery',
		label: 'Deliverables',
		owner: 'TL + JP',
	},
];
...
function branchFromOverview(project: Project | null, overview: Overview) {
	...
	for (const workspace of sortByTimeDesc(
		overview.runtimeWorkspaces.filter((workspace) => workspace.projectId === project.id),
	)) {
	...
	for (const run of sortByTimeDesc(
		overview.workflowRuns.filter((run) => run.projectId === project.id),
	)) {
		const metadata = asRecord(run.metadata);
		const diffSummary = asRecord(metadata.diffSummary);
		const branch =
			metadata.branch ?? metadata.branchName ?? diffSummary.branch ?? diffSummary.baseBranch;
		if (branch) return textValue(branch);
	}
	return 'not detected';
}
...
function roleMatches(blueprintRole: string, roleValue: string, idOrName = '') {
	...
	if (blueprintRole === 'developer')
		return ['developer', 'backend_engineer', 'frontend_engineer', 'implementer'].includes(
			normalizedRole,
		);
...
export function useWorkbenchData({
	...
	const projectWorkflows = useMemo(
		() =>
			sortByTimeDesc(
				project ? overview.workflows.filter((workflow) => workflow.projectId === project.id) : [],
			),
	...
	const projectWorkflowRuns = useMemo(
		() =>
			sortByTimeDesc(
				project ? overview.workflowRuns.filter((run) => run.projectId === project.id) : [],
			),
		[overview.workflowRuns, project],
	);
	const projectWorkflowEvents = useMemo(
		() =>
			sortByTimeDesc(
				project ? overview.workflowEvents.filter((event) => event.projectId === project.id) : [],
			),
		[overview.workflowEvents, project],
	);
	const projectEvents = useMemo(
		() =>
			sortByTimeDesc(
				project ? overview.events.filter((event) => event.projectId === project.id) : [],
			),
		[overview.events, project],
	);
	...
	const projectEvidence = useMemo(
		() =>
			sortByTimeDesc(
				project
					? overview.evidencePackages.filter((evidence) => evidence.projectId === project.id)
					: [],
			),
	...
	const projectArtifacts = useMemo(
		() =>
			sortByTimeDesc(
				project ? overview.artifacts.filter((artifact) => artifact.projectId === project.id) : [],
			),
		[overview.artifacts, project],
	);
	const projectTestResults = useMemo(
		() =>
			project ? overview.testResultRecords.filter((result) => result.projectId === project.id) : [],
		[overview.testResultRecords, project],
	);
	const projectAgentRuns = useMemo(
		() =>
			sortByTimeDesc(
				project ? overview.agentRuns.filter((run) => run.projectId === project.id) : [],
			),
	...
	const pendingApprovals = useMemo(
		() =>
			project
				? overview.actionRequests.filter(
						(item) => item.projectId === project.id && item.status === 'pending',
					)
				: [],
		[overview.actionRequests, project],
	);
	...
	const branch =
		detectedBranch === 'not detected'
			? t('app.workbench.workspace.branchNotDetected', 'not detected')
			: detectedBranch;

	const issueEvidenceId = String(objectRecord(taskRunResult?.evidencePackage)?.id ?? '');
	const selectedRunEvidence = selectedRunId
		? projectEvidence.find((evidence) => evidence.workflowRunId === selectedRunId)
		: null;
	const resolvedEvidenceId =
		issueEvidenceId || selectedRunEvidence?.id || projectEvidence[0]?.id || '';
	const activeEvidence = useMemo(
		() => projectEvidence.find((evidence) => evidence.id === resolvedEvidenceId) ?? null,
		[projectEvidence, resolvedEvidenceId],
	);
	...
	const teamRows = expertBlueprints.map((blueprint) => {
		...
		const configured =
			matchingProfiles.length > 0 || matchingCatalogAgents.length > 0 || matchingRuns.length > 0;
		const runtime =
			matchingProfiles[0]?.runtimeMode ??
			matchingCatalogAgents[0]?.providerId ??
			t('app.workbench.team.notConnected', 'not connected');
		return {
			...blueprint,
			configured,
			runtime,
			status:
				latestRun?.status ??
				(configured ? 'configured' : t('app.workbench.team.notConnected', 'not connected')),
			activity: latestRun
				? `${shortId(latestRun.id)} - ${formatTime(latestRun.updatedAt, t('app.workbenchEvidence.notRecorded', 'not recorded'))}`
				: '',
		};
	});
