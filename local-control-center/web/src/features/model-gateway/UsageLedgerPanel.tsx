...
export function UsageLedgerPanel({
	...
	return (
		<PanelShell title={t('ui.static.usage.ledger.bc792665', 'Usage history')}>
			<div className="field">
				<label htmlFor="usage-ledger-filter">{t('ui.static.usage.ledger.filter.c94a6388', 'Usage history filter')}</label>
				<input id="usage-ledger-filter" className="input" value={filter} onChange={(event) => onFilterChange(event.target.value)} placeholder={t('ui.static.filter.provider.model.role.runtime.workflow.or.agent.b9ddad3a', 'Filter provider, model, role, runtime, workflow or agent')} />
			</div>
			<DataTable rows={rows} empty={<EmptyState title={t('ui.static.no.usage.ledger.entries.0b082a29', 'No usage history entries')} body={t('ui.static.real.provider.and.runtime.calls.record.reported.usage.here.cec31f01', 'Real provider and runtime calls record reported usage here.')} />} columns={[
				{ key: 'time', label: t('ui.static.timestamp.19eabc96', 'Timestamp'), render: (row) => text(row.createdAt) },
				...
				{ key: 'runtime', label: t('ui.static.runtime.c4740e4c', 'Runtime'), render: (row) => text(row.runtimeType) },
				...
				{ key: 'task', label: t('ui.static.task.7bb0ddf9', 'Task'), render: (row) => text(row.taskId) },
				{ key: 'input', label: t('ui.static.input.tokens.92f7d222', 'Input tokens'), render: (row) => text(row.inputTokens, '0') },
				{ key: 'cached', label: t('ui.static.cached.input.36e191c9', 'Cached input'), render: (row) => text(row.cachedInputTokens, '0') },
				{ key: 'output', label: t('ui.static.output.tokens.b879f52d', 'Output tokens'), render: (row) => text(row.outputTokens, '0') },
