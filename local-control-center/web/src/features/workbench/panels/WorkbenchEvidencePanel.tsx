...
const CLOSED_PREVIEW: PreviewState = { open: false, title: '', text: '', loading: false, error: '' };
...
export function WorkbenchEvidencePanel({
	...
	useEffect(() => {
		...
		fetchEvidenceArtifact(token, securityPackageId, artifactId)
			...
			.catch((fetchError: unknown) => {
				if (active) setSecurityError(fetchError instanceof Error ? fetchError.message : t('app.workbench.evidence.securityError', 'Security findings could not be read.'));
			})
			.finally(() => {
	...
	const openPreview = async (packageId: string, artifactId: string, title: string) => {
		...
		try {
			...
			setPreview({ open: true, title, text: payload.text || t('app.workbench.evidence.binary', 'Binary artifact; download to inspect.'), loading: false, error: '' });
		} catch (fetchError) {
			setPreview({ open: true, title, text: '', loading: false, error: fetchError instanceof Error ? fetchError.message : t('app.workbench.evidence.previewError', 'Artifact could not be read.') });
		}
	};
	...
	const downloadArtifact = async (artifact: Overview['artifacts'][number]) => {
		...
		try {
			...
			setActionError(downloadError instanceof Error ? downloadError.message : t('app.workbench.evidence.downloadError', 'Artifact download failed.'));
		} finally {
			setDownloadingId('');
		}
	};
	...
	if (!evidencePackage) {
		return (
			<EmptyState
				...
				body={t('app.workbench.evidence.noPackageBody', 'Run a governed task to produce an evidence package; QA, hashes and findings appear here without leaving the workbench.')}
			/>
		);
	}
		...
		<div className="stack">
			<Surface title={t('app.workbench.evidence.packageTitle', 'Evidence package')}>
				<div className="inline">
					<Badge tone={toneForStatus(String(evidencePackage.qaVerdict))}>QA {String(evidencePackage.qaVerdict ?? 'not_started')}</Badge>
					<Badge>{String(evidencePackage.evidenceSource ?? 'operator_attested')}</Badge>
					<Badge>{scopedArtifacts.length} {t('app.workbench.evidence.artifacts', 'artifacts')}</Badge>
					{changedFiles !== null ? <span className="mono">{t('app.workbench.evidence.changedFiles', 'changed files')} {changedFiles}</span> : null}
			...
			<Surface title={t('app.workbench.evidence.qaTitle', 'QA results')}>
				<DataTable
					rows={scopedTests}
					empty={<EmptyState title={t('app.workbench.evidence.testsEmptyTitle', 'No QA results')} body={t('app.workbench.evidence.testsEmptyBody', 'QA commands record pass/fail results against an evidence package.')} />}
					columns={[
						{ key: 'command', label: t('app.workbench.evidence.colCommand', 'Command'), render: (row) => <span className="mono">{row.command}</span> },
						{ key: 'status', label: t('app.workbench.evidence.colStatus', 'Status'), render: (row) => <Badge tone={toneForStatus(String(row.status))}>{row.status}</Badge> },
						{ key: 'duration', label: t('app.workbench.evidence.colDuration', 'Duration ms'), render: (row) => <span className="tnum">{row.durationMs ?? 'n/a'}</span> },
					]}
				/>
			</Surface>
			...
			<Surface title={t('app.workbench.evidence.deliverablesTitle', 'Deliverables & hashes')}>
				...
				<DataTable
					...
					empty={<EmptyState title={t('app.workbench.evidence.deliverablesEmptyTitle', 'No deliverables yet')} body={t('app.workbench.evidence.deliverablesEmptyBody', 'Artifacts, patches and reports appear after real execution.')} />}
					columns={[
				...
				<div className="stack">
					...
					{securityLoading ? (
						<div className="empty-state" aria-busy="true">
							<strong>{t('app.workbench.evidence.securityLoading', 'Loading security findings')}</strong>
							<span>{t('app.workbench.evidence.securityLoadingBody', 'Findings are read from the linked artifact.')}</span>
						</div>
					) : null}
					{!securityArtifact ? (
						<EmptyState
							title={t('app.workbench.evidence.securityEmptyTitle', 'No security findings artifact')}
							body={t('app.workbench.evidence.securityEmptyBody', 'No security-findings.json artifact is linked to this evidence package.')}
						/>
					) : (
						<div className="inline">
			...
			<Surface title={t('app.workbench.evidence.callsTitle', 'Model & tool calls')}>
				<div className="stack">
					<div className="inline">
						<Badge>{modelCalls.length} {t('app.workbench.evidence.modelCalls', 'model calls')}</Badge>
						<Badge>{toolCalls.length} {t('app.workbench.evidence.toolCalls', 'tool calls')}</Badge>
					</div>
					<div>
						<div className="metric-label">{t('app.workbench.evidence.modelCalls', 'model calls')}</div>
						<pre className="artifact-preview">{redactVisibleText(modelCalls, '[]')}</pre>
					</div>
					<div>
						<div className="metric-label">{t('app.workbench.evidence.toolCalls', 'tool calls')}</div>
						<pre className="artifact-preview">{redactVisibleText(toolCalls, '[]')}</pre>
					</div>
					<div>
						<div className="metric-label">{t('app.workbench.evidence.policy', 'policy decisions & approvals')}</div>
						<pre className="artifact-preview">{redactVisibleText({ policyDecisions: evidencePackage.policyDecisions, approvals: evidencePackage.approvals }, '{}')}</pre>
					</div>
				</div>
			</Surface>
			...
			<Drawer label={preview.title || t('app.workbench.evidence.artifact', 'Artifact')} open={preview.open} onClose={() => setPreview(CLOSED_PREVIEW)}>
				{preview.loading ? (
					<div className="empty-state" aria-busy="true"><strong>{t('app.workbench.evidence.loading', 'Loading artifact')}</strong></div>
				) : preview.error ? (
