...
import type { useArtifactPreview } from './useArtifactPreview';
import type { useReviewDecision } from './useReviewDecision';
	...
	return (
		<div className="stack">
			<div className="inline">
				...
				<Badge>
					{selectedAction.expiresAt
						? `${t('app.review.expiresPrefix', 'expires')} ${selectedAction.expiresAt}`
			...
			<div
				className="diff-grid"
				role="table"
				aria-label={t('ui.static.action.request.scope.57c3a545', 'Action request scope')}
			>
				<DetailRow
					label={t('app.workbench.evidence.colCommand', 'Command')}
					value={redactVisibleText(
						selectedAction.command || t('app.review.notRecorded', 'not recorded'),
						t('app.review.notRecorded', 'not recorded'),
					)}
				/>
				<DetailRow
					label={t('ui.static.argv.2f50d8a9', 'Argv')}
					value={redactVisibleText((selectedAction.commandArgv ?? []).join(' '), '')}
				/>
				<DetailRow
					label={t('ui.static.workspace.4ca0a75c', 'Workspace')}
					value={`${selectedAction.workspaceId || t('app.review.notRecorded', 'not recorded')} ${selectedAction.workspacePath || ''}`.trim()}
				/>
				<DetailRow
					label={t('ui.static.runtime.c4740e4c', 'Runtime')}
					value={`${selectedAction.runtimeId || t('app.review.notRecorded', 'not recorded')} ${redactVisibleText(prettyJson(selectedAction.runtime), '')}`}
				/>
				<DetailRow label={t('ui.static.job.30c8cb83', 'Job')} value={selectedAction.jobId} />
				<DetailRow
					label={t('ui.static.project.f6f4da8d', 'Project')}
					value={selectedAction.projectId}
				/>
			</div>
			...
			<Surface
				title={t('ui.static.linked.evidence.packages.2437b4d6', 'Linked evidence packages')}
				flat
			>
				<DataTable
					...
					empty={
						<EmptyState
							...
							body={t(
								'ui.static.this.request.did.not.record.evidence.package.references.cfffb0d8',
								'This request did not record evidence package references.',
							)}
						/>
					}
					columns={[
			...
			<Surface title={t('ui.static.linked.artifacts.78fba781', 'Linked artifacts')} flat>
				<DataTable
					...
					empty={
						<EmptyState
							title={t('ui.static.no.linked.artifacts.4d728786', 'No linked artifacts')}
							body={t(
								'ui.static.diff.and.evidence.artifacts.must.be.attached.by.the.requesting.runtime.e7c8c851',
								'Diff and evidence artifacts must be attached by the requesting runtime.',
							)}
						/>
					}
					columns={[
								...
								return (
									<div className="inline" aria-busy={loading || downloading}>
										<button
											className="button"
											type="button"
											aria-label={`${t('app.review.previewArtifact', 'Preview artifact')} ${artifactDisplayName(row)}`}
											disabled={loading}
											onClick={() => void artifactPreview.openPreview(row)}
										>
											{loading
										...
										<button
											className="button"
											type="button"
											aria-label={`${t('app.review.downloadArtifact', 'Download artifact')} ${artifactDisplayName(row)}`}
											disabled={downloadBusy}
											onClick={() => void artifactPreview.download(row)}
										>
											{downloading
				...
				{artifactPreview.artifact ? (
					<div className="stack">
						<div className="inline">
							<Badge>
								{String(artifactPreview.artifact.kind ?? t('app.review.artifact', 'artifact'))}
							</Badge>
							<span className="mono">
								sha256{' '}
								{String(
									artifactPreview.payload?.hash ||
										artifactPreview.artifact.hash ||
										t('app.review.notRecorded', 'not recorded'),
								)}
							</span>
						</div>
						{artifactPreview.payload?.text ? (
							<pre className="artifact-preview">
								{redactVisibleText(artifactPreview.payload.text, '')}
							</pre>
						) : (
							<EmptyState
								title={
									artifactPreview.loadingId
			...
			{decision.patchGate?.required ? (
				<Surface
					title={t('ui.static.evidence.completeness.9a6c2f01', 'Evidence completeness')}
					flat
				>
					<div className="inline">
						<Badge tone={decision.patchGate.complete ? 'ok' : 'warn'}>
							{decision.patchGate.complete ? 'evidence_complete' : 'evidence_incomplete'}
						</Badge>
						<span>
							{decision.patchGate.complete
								? t(
										'app.review.patchGateComplete',
										'Approve patch can proceed after a human reason.',
									)
								: t(
										'app.review.patchGateBlocked',
										'Approve patch is blocked until evidence is complete.',
									)}
						</span>
					</div>
			...
			{decision.patchGate?.required ? (
				...
				>
					{decision.patchLoading ? (
						<EmptyState
							title={t('ui.static.loading.patch.artifact.19f1d8a6', 'Loading patch artifact')}
							body={t(
								'ui.static.the.diff.is.read.through.the.protected.evidence.artifact.endpoint.e9955146',
								'The diff is read through the protected evidence artifact endpoint.',
							)}
						/>
					) : null}
					{decision.patchError ? (
					...
					{!decision.patchArtifact ? (
						<EmptyState
							...
							body={t(
								'ui.static.a.patch.approval.requires.a.linked.diff.artifact.before.the.approve.patch.action.is.enabled.9f5a1b2c',
								'A patch approval requires a linked diff artifact before the approve patch action is enabled.',
							)}
						/>
					) : null}
					{decision.patchArtifact &&
					decision.patchPayload?.text &&
					!hasRealPatchChanges(decision.patchPayload.text) ? (
						<EmptyState
							title={t(
								'ui.static.patch.artifact.has.no.proven.changes.66c87331',
								'Patch artifact has no proven changes',
							)}
							body={t(
								'ui.static.the.patch.artifact.is.empty.or.malformed.so.approve.patch.remains.blocked.b0f0d46a',
								'The patch artifact is empty or malformed, so approve patch remains blocked.',
							)}
						/>
					) : null}
					{decision.patchPayload?.text ? (
			...
			{decision.patchGate?.required ? (
				...
				>
					{decision.securityLoading ? (
						<EmptyState
							title={t('ui.static.loading.security.findings.0b677e6f', 'Loading security findings')}
							body={t(
								'ui.static.security.evidence.is.read.through.the.protected.evidence.artifact.endpoint.7dd786fd',
								'Security evidence is read through the protected evidence artifact endpoint.',
							)}
						/>
					) : null}
					{decision.securityError ? (
						<div className="form-error" role="alert">
							{decision.securityError}
						</div>
					) : null}
					{!decision.securityArtifact ? (
						<EmptyState
							title={t(
								'ui.static.no.security.findings.recorded.5199261c',
								'No security findings recorded',
							)}
							body={t(
								'ui.static.patch.approval.requires.linked.non.blocking.security.findings.before.the.approve.patch.action.is.enabled.22bc9b5c',
								'Patch approval requires linked non-blocking security findings before the approve patch action is enabled.',
							)}
						/>
					) : null}
					{decision.securityArtifact &&
					decision.securityPayload?.text &&
					!securityFindingsAreNonBlocking(decision.securityPayload) ? (
						<EmptyState
							title={t(
								'ui.static.security.findings.are.blocking.or.unreadable.62a7c2ac',
								'Security findings are blocking or unreadable',
							)}
							body={t(
								'app.review.securityFindingsInvalid',
								'The security findings artifact must be valid JSON with no blocking policy decision.',
							)}
						/>
					) : null}
					{decision.securityPayload?.text ? (
