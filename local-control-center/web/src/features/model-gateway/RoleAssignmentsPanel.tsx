	...
	return (
		<PanelShell title={t('ui.static.role.assignments.1baf0a07', 'Role Assignments')}>
			<DataTable rows={rolePolicies} empty={<EmptyState title={t('ui.static.no.role.policies.a12a5674', 'No role policies')} body={t('ui.static.role.routing.policies.seed.during.startup.088f425d', 'Role routing policies seed during startup.')} />} columns={[
				...
				{ key: 'primary', label: t('ui.static.primary.provider.model.runtime.f0cce3eb', 'Primary provider/model/runtime'), render: (row) => listLabel(row.preferred) },
				{ key: 'fallbacks', label: t('ui.static.fallbacks.7ad29425', 'Fallbacks'), render: (row) => listLabel(row.fallback) },
				{ key: 'maxCost', label: t('ui.static.max.cost.task.b474ecd8', 'Max cost/task'), render: (row) => money(row.maxCostPerTaskUsd) },
				...
				{ key: 'approval', label: t('ui.static.approval.threshold.c49acc2d', 'Approval threshold'), render: (row) => money(row.requiresApprovalOverUsd ?? row.maxCostPerTaskUsd) },
