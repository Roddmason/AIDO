...
- Existing `cost_usage` is updated only as a temporary read-model for older
  ...
  caller's project (AI execution passes it); a row recorded without one keeps
  `projectId = null` in the overview contract. The Control Center sums every
  `cost_usage` row, so each call must appear once: the model gateway skips the
  compatibility row (`record_cost_usage=False`) because its `model_call` row,
  the one budget enforcement reads, already carries the call's cost (zero
  included).
