...
REPORT_SCHEMA = "aido.codex-cli-release-validation.v1"
VALIDATION_MARKER = "codex release validation passed"
DEFAULT_TITLE = "Codex CLI release validation patch"
DEFAULT_ISSUE_TEXT = f"""Release validation smoke for the Codex CLI adapter.
...
def required_environment_errors(environ: Mapping[str, str]) -> list[str]:
    errors: list[str] = []
    if not str(environ.get("AIDO_CODEX_COMMAND") or "").strip().strip('"'):
        errors.append("AIDO_CODEX_COMMAND is required for Codex CLI release validation.")
    if str(environ.get("AIDO_ENABLE_CLI_RUNTIMES") or "").strip().lower() != "true":
        errors.append("AIDO_ENABLE_CLI_RUNTIMES=true is required for Codex CLI release validation.")
    return errors
...
def release_result_contract_errors(result: Mapping[str, Any]) -> list[str]:
    ...
    if runtime.get("id") != "codex_cli" or runtime.get("kind") != "cli":
        errors.append("Runtime must be codex_cli with cli kind.")
    ...
    if runtime_result.get("status") != "completed" or runtime_result.get("returnCode") != 0:
        errors.append("Codex CLI runtime execution did not complete with return code 0.")
    ...
    if not changed_files or patch_size <= 0 or not diff_summary.get("patchArtifactId"):
        errors.append("Codex issue_to_patch must produce a non-empty patch artifact.")
    ...
    if evidence.get("runtimeId") != "codex_cli":
        errors.append("Evidence package must be linked to codex_cli.")
    ...
    if not tool_calls:
        errors.append("Evidence package must include the Codex CLI tool call.")
...
def _default_work_root() -> Path:
    return REPO_ROOT / ".tmp" / "codex-cli-release-validation" / _utc_slug()
...
def _create_validation_git_repo(work_root: Path) -> Path:
    ...
    (repo_path / "README.md").write_text("# Codex CLI Release Validation\n", encoding="utf-8")
    ...
    (tests_dir / "test_release_patch.py").write_text(
        "\n".join(
            [
                ...
                "",
                "",
                "def test_codex_release_validation_patch_exists() -> None:",
...
def _enable_codex_provider(connection: Any) -> None:
    timestamp = utc_now()
    connection.execute(
        """
...
def run_release_validation(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    codex_command = str(os.environ["AIDO_CODEX_COMMAND"]).strip().strip('"')
    resolved_codex = _resolve_codex_command(codex_command)
    if not resolved_codex:
        return 1, {
            ...
            "errors": [f"Codex CLI command not found from AIDO_CODEX_COMMAND: {codex_command}"],
        }
    ...
    try:
        ...
        runtime.init()
        _enable_codex_provider(runtime.connection)
        project = ProjectsRepository(runtime.connection).create_project(
            name="Codex CLI Release Validation",
            path=source_repo,
            template_id="other",
            create_directory=False,
            source="release_validation",
            metadata={"codexCommand": resolved_codex, "schema": REPORT_SCHEMA},
        )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run real Codex CLI release validation through issue_to_patch.")
