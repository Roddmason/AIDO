/**
 * Shell-level bodies for the Settings modal (runtime, workspaces, advanced/developer) plus
 * the shared ConsoleLink affordance. Each renders a compact body over real control-plane
 * data and deep-links to the heavy operational consoles (Policy, Evidence, Audit,
 * Model Gateway…) rather than duplicating them. Dense catalogs stay behind a Disclosure so
 * no section opens on a table. The modal's section registry (sections.tsx) imports these;
 * the per-section panels (general, appearance, worker, teams, goal, routing, quality,
 * research, internet, credentials, plugins) live in their own sibling modules.
 *
