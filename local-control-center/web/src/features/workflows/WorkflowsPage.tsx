...
export function WorkflowsPage({
	...
	const linked = useMemo<WorkflowLinks>(() => {
		...
		const runs = overview.workflowRuns.filter(
			(run) => String(run.workflowId ?? '') === selectedWorkflow.id,
		);
		...
		const agentRuns = overview.agentRuns.filter(
			(run) =>
				runIds.has(String(run.workflowRunId ?? '')) ||
				stepIds.has(String(run.workflowStepId ?? '')),
		);
		...
		return {
			...
			events: overview.workflowEvents.filter(
				(event) =>
					String(event.workflowId ?? '') === selectedWorkflow.id ||
					runIds.has(String(event.workflowRunId ?? '')),
			),
	...
	return (
		<>
			...
			<Surface title={t('ui.static.workflow.graph.32f4369a', 'Workflow graph')}>
				{selectedWorkflow && nodes.length ? (
					<div
						...
						aria-label={`${t('app.workflows.ariaGraphFor', 'Workflow graph for')} ${selectedWorkflow.title}`}
					>
					...
					<EmptyState
						title={t('ui.static.no.workflow.steps', 'No workflow steps')}
						body={t(
							'ui.static.select.or.start.workflow.run.before.reading.graph',
							'Select or start a workflow run before reading a graph.',
						)}
					/>
				)}
			</Surface>
			<div className="grid two">
				...
				<Surface title={t('ui.static.selected.workflow.steps', 'Selected workflow steps')}>
					<DataTable
						...
						empty={
							<EmptyState
								title={t('ui.static.no.selected.workflow.steps', 'No selected workflow steps')}
								body={t(
									'ui.static.inspect.workflow.with.run.to.see.step.plan',
									'Inspect a workflow with a run to see its step plan.',
								)}
							/>
						}
						columns={[
							...
							},
							{
								...
								render: (row) =>
									row.agentProfileId ?? t('app.workflows.agentUnassigned', 'unassigned'),
							},
						]}
					/>
				</Surface>
			...
			<Drawer
				label={t('ui.static.workflow.inspector.d8560d75', 'Workflow inspector')}
			...
			>
				<div className="drawer-body">
					{selectedWorkflow ? (
						<>
							<Surface title={selectedWorkflow.title} flat>
								<div className="inline">
									<Badge tone={toneForStatus(selectedWorkflow.status)}>
									...
									columns={[
										{
											...
											label: t('ui.static.run.44c29edb', 'Run'),
											render: (row) => (
												<span className="mono">{shortId(String(row.id ?? ''))}</span>
											),
										},
										{
											key: 'status',
										...
										},
										{
											...
											render: (row) => {
												...
												return (
													<span className="mono">
														{String(
															diff.state ?? t('app.workflows.diffNotCaptured', 'not captured'),
														)}{' '}
							...
							<Surface
								title={t('ui.static.evidence.and.tests.0c061aed', 'Evidence and tests')}
								flat
							>
								<DataTable
									...
									columns={[
										...
										},
										{
											...
											render: (row) =>
												String(Array.isArray(row.diffRefs) ? row.diffRefs.length : 0),
										},
										{
											key: 'completeness',
								...
								<DataTable
									...
									columns={[
										...
										},
										{
											...
											render: (row) => (
												<span className="mono">{shortId(String(row.hash ?? ''))}</span>
											),
										},
										{
											key: 'action',
							...
							<Surface title={t('ui.static.jobs.and.leases.4768364f', 'Jobs and leases')} flat>
								<div className="field">
									<label htmlFor="workflow-job-mutation-reason">
										{t('app.copy.standalone.3', 'Queue change reason')}
									</label>
									<textarea
										id="workflow-job-mutation-reason"
										className="textarea"
												...
												return (
													<div className="inline" aria-busy={busy}>
														<button
															...
															disabled={blocked}
															onClick={() =>
																void runJobMutation('retry', row.id, jobMutationReason)
															}
														>
															{busy
														...
														<button
															...
															disabled={blocked}
															onClick={() =>
																void runJobMutation('cancel', row.id, jobMutationReason)
															}
														>
															{busy
								...
								<DataTable
									...
									empty={
										<EmptyState
											title={t('ui.static.no.job.runs.4e3be714', 'No job runs')}
											body={t(
												'ui.static.worker.lease.attempts.appear.here.when.jobs.execute.35553b89',
												'Worker lease attempts appear here when jobs execute.',
											)}
										/>
									}
									columns={[
										...
										{
											key: 'summary',
											label: t('ui.static.summary.b5473fa1', 'Summary'),
											render: (row) => redactVisibleText(row.summary),
										},
									]}
								/>
							</Surface>
						...
						<EmptyState
							title={t('ui.static.no.workflow.selected.215ac484', 'No workflow selected')}
							body={t(
								'ui.static.choose.a.workflow.to.inspect.its.linked.records.4f7e51a2',
								'Choose a workflow to inspect its linked records.',
							)}
						/>
					)}
				</div>
