...
3. **i18n & honesty debt** — large surfaces (`pages.tsx`, Model Gateway, Runs, Home, parts of Settings, `StatusBar`, the shell drawers/command labels) bypass the `t()` catalog; and `state.connected` is hard-wired `false`, leaving the StatusBar permanently "polling" and one command + shortcut permanently dead.
...
| "connected" | **No SSE/WebSocket.** `state.connected` is initialized `false` and **never set `true`** (`useControlPlane.ts:32`). The honest *polling* posture is correct, but the flag is mis-modeled (Section 4.4, H1). |
...
| # | Problem | Evidence | Direction |
| --- | --- | --- | --- |
| H1 | **`state.connected` is hard-wired `false` forever.** It is initialized `false` and never set `true` on a successful overview load, so the StatusBar can never read "API connected" (always "polling"), and the "Refresh runtime health" command + its `Ctrl+Alt+R` shortcut are permanently disabled ("Control plane offline"). Dead UI masquerading as a gate. | `useControlPlane.ts:32,72-91`; `StatusBar.tsx:41-42`; `commandActions.ts:181`; `App.tsx:178` | Set `connected:true` in the success `setState` (and `false` in the catch). This stays honest — a successful authenticated overview *is* a real reachability signal — and **does not** imply SSE; keep "polling" framing only if it means "no live stream." |
...
| H8 | **Dead/duplicate styling.** Light theme is fully defined but unreachable (nothing sets `data-theme`, no `prefers-color-scheme` fallback); ~120 lines of `task-/run-/setting-/workspace-card` CSS have no consumer; duplicate `.workbench-layout`/`-ide-layout` and `.tabs`/`.panel-tabs`; a legacy token-alias block persists; `Badge`/`StatusDot` types omit the `pending` tone the palette defines. | `tokens.css:157-207,128-154,96`; `components.css:155-257,387-397`; `layout.css:528-534`; `primitives.tsx:9,17` | Wire a theme toggle or drop the light block; delete dead card CSS; consolidate duplicate classes; add the `pending` tone; migrate + delete the alias block. |
